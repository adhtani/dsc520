...
total_points_by_column <- colSums(grades)
## Display total_points_by_column
total_points_by_column

## Total points for each student using `rowSums()`
total_points_by_row <- rowSums(grades)
## Display total_points_by_row
total_points_by_row
...
## Created a DataFrame with 9 rows and 5 columns
...
## Returns 6 rows where ring_bearer is TRUE
head(ringbearers_df)